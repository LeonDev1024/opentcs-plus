...
| 字段 | 用途 | 说明 |
|------|------|------|
| `FactoryModel.scale` | 比例尺 | px/m，默认 50px = 1米 |
| `FactoryModel.coordinateSystem` | 坐标系 | RIGHT_HAND / LEFT_HAND |
| `FactoryModel.lengthUnit` | 长度单位 | METER / CENTIMETER / MILLIMETER |
| `NavigationMap.originX/Y` | PGM导入用 | 仅数据导入时使用，识别原始图纸坐标 |
| `Point.x/y/z` | 统一坐标 | 前端显示和后端计算共用，无需转换 |

### 3.3 设计原理

1. **前端画布原点固定 (0,0)** - 始终不变
2. **NavigationMap.origin** - 仅用于数据导入时，识别原始 PGM 地图的坐标位置
3. **Point 坐标统一** - 存储时保持一致，减少转换开销
4. **跨楼层计算** - 直接使用 Point 的 x/y/z 坐标，配合楼层号（floorNumber）计算
...
CREATE TABLE navigation_map (
    ...
    floor_number INT COMMENT '楼层号（负数表示地下，null表示室外/公共区域）',
    map_type VARCHAR(50) NOT NULL DEFAULT 'INDOOR' COMMENT '地图类型：INDOOR/OUTDOOR/MIXED',
    -- PGM导入时使用：识别原始图纸的原点位置
    origin_x DECIMAL(12,4) DEFAULT 0 COMMENT 'PGM地图原点的全局X坐标（仅导入时使用）',
    origin_y DECIMAL(12,4) DEFAULT 0 COMMENT 'PGM地图原点的全局Y坐标（仅导入时使用）',
    properties JSON COMMENT '扩展属性',
    -- 审计字段
    create_dept BIGINT,
    create_by BIGINT,
