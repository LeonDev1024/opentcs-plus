#!/usr/bin/env bash
# 本地开发数据库快捷命令
# 用法: ./db/dev-db.sh rebuild|migrate|info|snapshot|validate|seed
set -euo pipefail
...
rebuild_database() {
  ...
  flyway_mvn migrate
  mysql_import "$SCRIPT_DIR/seed/dev_bootstrap.sql"
  echo "Rebuild complete (admin / admin123)."
}

usage() {
  cat <<'EOF'
用法: ./db/dev-db.sh <command>
...
case "$cmd" in
  ...
  seed)
    mysql_import "$SCRIPT_DIR/seed/demo_factory_data.sql"
