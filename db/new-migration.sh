#!/usr/bin/env bash
# 创建新的 Flyway 版本化迁移文件
# 用法: ./db/new-migration.sh 1.0.3 add_vehicle_battery_column
set -euo pipefail
...
TARGET="$SCRIPT_DIR/migration/V${VERSION}__${DESC}.sql"
...
echo "Created: $TARGET"
echo "Next:"
echo "  ./db/dev-db.sh migrate"
echo "  ./db/dev-db.sh snapshot"
