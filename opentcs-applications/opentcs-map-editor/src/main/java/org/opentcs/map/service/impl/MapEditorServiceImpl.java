...
public class MapEditorServiceImpl implements IMapEditorService {
    ...
    public PlantModelBO load(LoadModelVO loadModelVO) {
        PlantModelEntity plantModel = plantModelService.getById(loadModelVO.getModelId());
        ...
        // 停靠点集合
        List<PointEntity> points = pointService.selectAllPointByPlantModelId(plantModel.getId());
        plantModelBO.setPoints(new HashSet<>(points));
        // 路径集合
        List<PathEntity> paths = pathService.selectAllPathByPlantModelId(plantModel.getId());
        plantModelBO.setPaths(new HashSet<>(paths));
        // 业务点位集合
        List<LocationEntity> locations = locationService.selectAllLocationByPlantModelId(plantModel.getId());
        plantModelBO.setLocations(new HashSet<>(locations));
        // 交通规则区域集合
        List<BlockEntity> blocks = blockService.selectAllBlockByPlantModelId(plantModel.getId());
    ...
    public boolean save(PlantModelBO plantModelBO) {
        ...
        if (StrUtil.equals(plantModelBO.getModelVersion(), "1.0")) {
            ...
            // 复制地图元素
            PlantModelEntity plantModel = new PlantModelEntity();
            ...
            // 保存地图元素
            Set<PointEntity> points = plantModelBO.getPoints();
            ...
            Set<PathEntity> paths = plantModelBO.getPaths();
            ...
            Set<LocationEntity> locations = plantModelBO.getLocations();
            ...
            Set<BlockEntity> blocks = plantModelBO.getBlocks();
