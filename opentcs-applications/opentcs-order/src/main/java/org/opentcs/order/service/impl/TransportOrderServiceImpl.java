...
public class TransportOrderServiceImpl extends ServiceImpl<TransportOrderMapper, TransportOrderEntity> implements TransportOrderService {
    ...
    public Map<String, Object> getOrderStatistics() {
        ...
        // 各状态订单数
        long rawOrders = this.count(new LambdaQueryWrapper<>(TransportOrderEntity.class).eq(TransportOrderEntity::getState, "RAW"));
        long activeOrders = this.count(new LambdaQueryWrapper<>(TransportOrderEntity.class).eq(TransportOrderEntity::getState, "ACTIVE"));
        long finishedOrders = this.count(new LambdaQueryWrapper<>(TransportOrderEntity.class).eq(TransportOrderEntity::getState, "FINISHED"));
        long failedOrders = this.count(new LambdaQueryWrapper<>(TransportOrderEntity.class).eq(TransportOrderEntity::getState, "FAILED"));
