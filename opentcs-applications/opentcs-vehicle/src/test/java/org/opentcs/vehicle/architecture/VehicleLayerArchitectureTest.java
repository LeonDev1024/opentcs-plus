 ...
 *   <li>Controller 不得绕过 ApplicationService 直接调用 DomainService</li>
...
class VehicleLayerArchitectureTest {
    ...
    /**
     * Controller 不得直接调用 persistence 层 DomainService（须通过 ApplicationService 中转）。
     */
    @ArchTest
    static final ArchRule controller_must_not_call_domain_service_directly =
            noClasses()
                    ...
                    .because("Controller 须通过 ApplicationService 调用业务逻辑，不得直接引用 DomainService");
